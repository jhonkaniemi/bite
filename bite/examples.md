...
Roe deer uses four different core modules of BITE: 1) Introduction, 2) Colonization, 3) Population dynamics and 4) Impact. Roe deer as a roaming browser is an example of an agent without a specific dispersal kernel and introduction to the entire landscape using `BiteDistribution` item. As the cell size of 100ha covers quite a variety of forest stands, we aggregate the tree information from the entire cell and check that the host with height <1.3m is presents. The carrying capacity....
